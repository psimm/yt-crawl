...
def test_help_and_offline_dashboard_do_not_configure_logfire(
    monkeypatch, tmp_path
) -> None:
    calls = []
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "yt_searchapi.dashboard.render_dashboard",
        lambda _run_dir, output: output,
    )
    monkeypatch.setattr(cli.webbrowser, "open", lambda _url: True)

    help_result = CliRunner().invoke(app, ["--help"])
    dashboard_result = CliRunner().invoke(
        app,
        [
            "dashboard",
            "--run-dir",
...
def test_dashboard_opens_resolved_file_uri_by_default(monkeypatch, tmp_path) -> None:
    output = tmp_path / "dashboard.html"
    opened_urls: list[str] = []
    monkeypatch.setattr(
        "yt_searchapi.dashboard.render_dashboard",
        lambda _run_dir, _output: output,
    )
    monkeypatch.setattr(
        cli.webbrowser,
        "open",
    ...
    result = CliRunner().invoke(app, ["dashboard", "--run-dir", str(tmp_path)])

    assert result.exit_code == 0
    assert opened_urls == [output.resolve().as_uri()]
    assert "Opened the dashboard in your default browser." in result.output


def test_dashboard_no_open_skips_browser(monkeypatch, tmp_path) -> None:
    output = tmp_path / "dashboard.html"
    monkeypatch.setattr(
        "yt_searchapi.dashboard.render_dashboard",
        lambda _run_dir, _output: output,
    ...
    monkeypatch.setattr(
        cli.webbrowser,
        "open",
        lambda _url: pytest.fail("dashboard --no-open must not launch a browser"),
    )

    result = CliRunner().invoke(
        app,
        ["dashboard", "--run-dir", str(tmp_path), "--no-open"],
    )

    assert result.exit_code == 0
    assert "Browser opening disabled" in result.output


def test_dashboard_keeps_generated_file_when_browser_cannot_open(
    monkeypatch, tmp_path
) -> None:
    output = tmp_path / "dashboard.html"
    monkeypatch.setattr(
    ...
    assert "Could not open the dashboard automatically" in result.output
    assert "dashboard.html manually." in result.output
